...
export default function WorkoutSession() {
  ...
  const saveSession = async () => {
    ...
    if (allSets.length > 0) {
      const { error: setsErr } = await supabase.from("session_sets").insert(
        allSets.map((s) => ({ ...s, session_id: session.id }))
      );
      if (setsErr) {
  ...
  if (loading) return <Layout><p className="text-sm text-muted-foreground">Carregando...</p></Layout>;
          ...
          <div className="space-y-4">
            {exercises.map((ex, exIdx) => (
              <Card key={ex.id}>
                ...
                <CardContent>
                  {ex.sets.length === 0 ? (
                    <p className="text-xs text-muted-foreground">Nenhuma série.</p>
                  ) : (
            ...
            <Button onClick={saveSession} disabled={saving} className="w-full">
